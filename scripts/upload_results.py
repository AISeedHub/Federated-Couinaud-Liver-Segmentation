...
실제 환자 ID는 export 폴더에 없음(export_results.py가 Case N으로 치환, _id_map_LOCAL_ONLY.json은 client_ 폴더에만 존재).
  ...
  python scripts/upload_results.py --exp exp4c --site A [--url http://SERVER_IP:9598/upload]
...
def main():
    ...
    ap.add_argument("--url", default=None, help="기본: configs/<exp>.yaml의 upload_url, 없으면 http://SERVER_IP:9598/upload"); ap.add_argument("--out", default="outputs"); ap.add_argument("--run", required=True); a = ap.parse_args()
    if not a.url:
